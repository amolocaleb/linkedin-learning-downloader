...
Course = namedtuple(
    "Course", ["name", "slug", "description", "unlocked", "chapters"])
...
def chapter_dir(course: Course, chapter: Chapter):
    ...
    chapter_path = os.path.join(
        BASE_DOWNLOAD_PATH, clean_dir_name(course.name), folder_name)
    return chapter_path
...
async def fetch_video(course: Course, chapter: Chapter, video: Video):
    ...
    async with aiohttp.ClientSession(headers=HEADERS, cookie_jar=COOKIE_JAR) as session:
        ...
        for _ in range(tries):
            try:
                resp = await session.get(video_url, proxy=PROXY, headers=HEADERS, timeout=-1)
                data = await resp.json()
